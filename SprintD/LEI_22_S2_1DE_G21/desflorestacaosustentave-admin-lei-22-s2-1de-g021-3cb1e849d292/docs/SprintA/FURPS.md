...
- _Localization: Should allow the SNS user to choose which vaccination center he wants to schedule the vaccination._
- _Email: Should send an email when the recovery period ends._
- _SMS: Should send an SMS when the recovery period ends._
- _Printing: All the images/figures produced during the software development process should be recorded in SVG format._
- _Security: Password authentication for every user. The password must hold seven alphanumeric characters, including three capital letters and two digits. Also, only the nurses are allowed to access all user’s health data._
